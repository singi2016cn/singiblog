...
theme = ( function ( theme, $, window, document ) {
    ...
    var pageLoaded = function() {
			...
			function aequilate() {
				...
				$( 'ul.menu-main > li ul, ul.menu-main > li a' ).css( 'width', (windowWidth - 48 )/menuNum + 'px' );	
			...
			$toggle.sidr({
			  ...
			  onOpen: function( ev ) {
				...
				var m_logo = $( '.sidr-class-mobile-brand img' ).attr( 'src' );
				if ( typeof m_logo !== typeof undefined && m_logo != '' ) {
					if ( m_logo.indexOf( 'blank.gif' ) >= 0 ) $( '.sidr-inner' ).css( 'margin-top', '-40px' );
				}			  
		    ...
		    $( '.admin-bar .menu-toggle' ).addClass( 'adminbar-pos' );
			...
			$window.on('scroll', function() {
				...
				if ( $window.scrollTop() >= 46 ) {	
					$( '.admin-bar .menu-toggle' ).addClass( 'adminbar-pos-scroll' );
				}else{
					$( '.admin-bar .menu-toggle' ).removeClass( 'adminbar-pos-scroll' );
				};
			...
			$( '#menu-search-btn' ).on( 'click', function() {
				...
				$( 'ul.menu-main > li ul, ul.menu-main > li a' ).css( 'width', ($window.width() - 48 - 170 )/menuNum + 'px' );	
			...
			$( '.menu-right' ).on( 'mouseleave', function(){
				...
				$( 'ul.menu-main > li ul, ul.menu-main > li a' ).css( 'width', ($window.width() - 48 )/menuNum + 'px' );	
			});	
...
theme = ( function ( theme, $, window, document ) {
    ...
    var documentReady = function( $ ) {
		...
		if ( !$( 'body' ).hasClass( 'custom-background' ) ) {
			...
			$( '.footer-main-container, .container' ).removeClass( 'has-bg' );	
		} else {
			$( '.footer-main-container, .container' ).addClass( 'has-bg' );	
		}
...
theme = ( function ( theme, $, window, document ) {
    ...
    var documentReady = function( $ ) {
		...
		$( '.title-style-fieldset' ).each( function()  {
			
			if ( $( this ).find( 'h5' ).text().trim().length == 0 ) {
