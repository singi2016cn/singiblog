...
if( !function_exists( 'shadower_commentopen_contact_do' ) ) {
	
	add_action( 'wp_enqueue_scripts', 'shadower_commentopen_contact_do' );
	function shadower_commentopen_contact_do(){
...
if ( !function_exists( 'shadower_custom_comment_form' ) ) {
	...
	function shadower_custom_comment_form( $form_options ) {
		...
		$form_options = array( 
			...
			'comment_notes_before' => ( Shadower_Core::check_curpage_from_template( get_the_ID(),array( 'tmpl-contact.php' ) ) ? '' : '<p class="comment-notes">'.shadower_wp_kses( __( 'Your email address will not be published.', 'shadower' ) ).'</p>' ),
			...
			*/
			...
			'title_reply'          => ( Shadower_Core::check_curpage_from_template( get_the_ID(),array( 'tmpl-contact.php' ) ) ? '' : shadower_wp_kses( __( 'Leave a Reply', 'shadower' ) ) ),
			...
			'label_submit'         => esc_attr__( 'Post Comment', 'shadower' )
		 );
...
if ( !function_exists( 'shadower_comment_form_after' ) ) {
	...
	function shadower_comment_form_after() {
		...
		$html = preg_replace(
			...
			'<h3 '.( Shadower_Core::check_curpage_from_template( get_the_ID(),array( 'tmpl-contact.php' ) ) ? 'style="display:none;"' : '' ).' id="reply-title"\1>\2</h3>',
			$html
		);
		echo $html;
	}
